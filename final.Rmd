...
for (lc in labels2) {
  ...
  col_breaks <- seq(d_min, d_max, length=11)
  data[,lc] = cut(as.numeric(col_data),breaks=col_breaks,labels=c(1:10))
}
```
...
```
```{r}
train.predictor <- data.train[,labels[-1]]
train.mortstat <- data.train[,labels[1]]

test.predictor <- data.test[,labels[-1]]
test.mortstat <- data.test[,labels[1]]
```

#### Random Forest
```{r}
mortstat_randomforest <- randomForest(as.factor(mortstat) ~ ., data = data.train, mtry = 19, ntree = 500)
```
